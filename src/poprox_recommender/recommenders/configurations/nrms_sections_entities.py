...
from poprox_recommender.components.joiners.score import MinMaxScores, ScoreFusion
...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    # Drop articles dominated by strongly-disliked entities (rating 1). This runs before
    # embedding so it affects every section, the same way TopicPrefsFilter's disliked-topic
    # rule does for topics.
    f_entity = builder.add_component(
        "entity-filter",
        EntityPrefsFilter,
        candidates=f_candidates,
    ...
    e_candidates = builder.add_component("candidate-embedder", NRMSArticleEmbedder, ae_config, article_set=f_entity)
